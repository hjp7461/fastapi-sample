...
상품의 생성/수정/삭제와 재고 변경. **모두 `require_staff_or_admin` 으로 보호된다 — STAFF + ADMIN 통과 (CUSTOMER 는 403).** 권한 정책 결정 배경은 [`RUNBOOK §9.4`](../RUNBOOK.md) (B-2 변형 / PR #26).

> 모든 다이어그램의 **인증/권한 단계는 동일** 하므로 첫 다이어그램(상품 생성)에 한 번 펼쳐 그리고, 이후에는 "권한 검사 통과 가정 (`can_manage_products()`)" 으로 축약한다. 인증 + 권한 가드 상세 흐름은 [`인증_및_권한.md`](./인증_및_권한.md) 참조.
...
sequenceDiagram
    ...
    participant Guard as require_staff_or_admin
    ...
    Router->>Guard: Depends(require_staff_or_admin)
    Guard->>User: Depends(get_current_user)
    Note over User: JWT 디코드 + 사용자 조회

    alt 인증/권한 실패
        User--xGuard: HTTPException
        Guard-->>Router: 전파
        Router-->>Client: 401 / 400 / 403 (CUSTOMER)
    else 권한 통과 (STAFF / ADMIN)
        Guard-->>Router: User
...
sequenceDiagram
    ...
    Router->>Guard: Depends(require_staff_or_admin)
    Note over Guard: 권한 검사 통과 가정<br/>(can_manage_products())
