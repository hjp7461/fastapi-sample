...
| 의존성     | `get_self_or_admin` (path parameter `user_id` 자동 주입 + 권한 검증)              |
| 권한 정책  | **본인 또는 관리자만**                                                            |
| 성공       | `200 OK` + `UserResponse`                                                         |
...
- **권한 검사를 의존성으로 추출** — 라우터 본문은 비즈니스 로직 (service 호출 + 404 매핑) 만 담당. 권한 분기는 `get_self_or_admin` 의존성이 책임진다.
- **권한 평가가 존재 확인보다 먼저** 다. 일반 사용자가 ID 를 바꿔가며 `403` vs `404` 응답 차이로 사용자 ID 의 존재 여부를 열거하는 공격을 차단한다.
  ...
  - 관리자만 `404` 를 받을 수 있음
- FastAPI 가 `get_self_or_admin(user_id: int, ...)` 의 `user_id` 를 라우터의 path parameter 와 동일 이름으로 자동 주입.
