...
def _register_envelope_components(schema: dict[str, Any]) -> None:
    """components/schemas 에 envelope 모델 추가 (Generic SuccessEnvelope 제외 —
    인스턴스화된 형태가 paths 안에 inline 으로 들어가므로 별도 등록 불필요)."""
    ...
    for path, methods in schema.get("paths", {}).items():
        ...
        for method, op in methods.items():
            ...
            for status_code, response in responses.items():
                ...
                if status_code == "204":
                    continue
                _wrap_response_content(response)


def _wrap_response_content(response: dict[str, Any]) -> None:
